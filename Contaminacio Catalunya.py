...
st.title("🌫️ Contaminació Catalunya")
...
if tile_option == "Stamen Toner":
    ...
    m = folium.Map(location=map_center, zoom_start=10, tiles=tile_option)
...
st.subheader("📊 Average Contamination by Hour")
...
# -------------------------
# Optional: Yearly and monthly averages (if data exists)
if "YEAR" in df.columns and "MONTH" in df.columns:
    yearly_avg = df.groupby("YEAR")["AVG_CONTAM"].mean().reset_index()
    monthly_avg = df.groupby("MONTH")["AVG_CONTAM"].mean().reset_index()
    ...
    plt.tight_layout()
    st.subheader("📊 Average Contamination by Year & Month")
    st.pyplot(fig)
    plt.clf()
